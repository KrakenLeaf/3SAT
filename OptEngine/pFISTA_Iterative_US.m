function [ x, X_stack ] = pFISTA_Iterative_US( Y, H, S, L, Pw, Params )
...
% Syntax:
% -------
% [ x, X_stack ] = pFISTA_Iterative_US( Y, H, Params )
%
% Inputs:
% -------
...
%% Initialization
% -----------------------------------------------------------------------------------------------------------------------
TotIter = Params.Iterative.NumOfSteps;                  % Total number of "enveloping" iterations
eps     = Params.Iterative.Eps;                         % Regularization parameter of the "enveloping" iterative scheme
...
%% Calculate constants
% -----------------------------------------------------------------------------------------------------------------------
if VERBOSE >= 3; disp('Performing constant calculations...'); ConstTime = tic; end;
% Caclculate the vector v
if VERBOSE >= 3; fprintf('v_calc: ');t1 = tic; end;
% if LargeScaleFlag == 1
...
if VERBOSE >= 3; disp('done.'); toc(ConstTime); end;

%% Iterations
% -----------------------------------------------------------------------------------------------------------------------
for ii = 1:TotIter 
    % Output to screen
    if VERBOSE >= 3; disp(['pFISTA_Iterative scheme: Iteration #' num2str(ii)]); disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'); end;
...
if VERBOSE >= 3; disp('Done performing pFISTA_Iterative.'); end;
